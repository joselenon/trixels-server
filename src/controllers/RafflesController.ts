...
import ProcessRaffleBetsService from '../services/ProcessRaffleBetsService';
...
class RafflesController {
  ...
  async createRaffle(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      RaffleUtils.verifyRaffleCreationPayloadValidity(raffleCreationPayload);

      const raffleIdResponse = await new CreateRaffle({ userId: userDocId, raffleCreationPayload }).create();
  ...
  async addRaffleTicketBuyToQueue(req: Request, res: Response, next: NextFunction) {
    try {
      const nowTimeInSeconds = new Date().getTime();
      ...
      await ProcessRaffleBetsService.addRaffleTicketBuyToQueue(buyRaffleTicketPayload, userDocId, nowTimeInSeconds);
